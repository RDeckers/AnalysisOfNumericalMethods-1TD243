...
therefore
\begin{align}
 \Aboxed{V_\omega = \frac{1}{6h\omega}\left[-\sin(2h\omega)+8\sin(h\omega)\right]}
\end{align}
\begin{figure}[!hhp]
\centering
...
\begin{align}
\Aboxed{e^{-i\omega t}\left(\frac{h^4\omega^4}{30}-2\right)i\omega t}
\end{align}
...
\begin{align}
 &\bar{\vec u}^+ = \bar{\vec u} + \frac{k}{2}\left[AD_+D_-\bar{ \vec u}+AD_+D_-\bar{ \vec u}^+]\nn
